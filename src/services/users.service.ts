...
export const usersService = {
  ...
  async getXpLogs(params?: { page?: number; limit?: number }): Promise<XpLog[]> {
    const response = await api.get<XpLog[]>(API_ROUTES.USERS.XP_LOGS, { params })
    return response.data
  },

  async getEnergyLogs(params?: { page?: number; limit?: number }): Promise<EnergyLog[]> {
    const response = await api.get<EnergyLog[]>(API_ROUTES.USERS.ENERGY_LOGS, { params })
    return response.data
  },

  // Notificações
  async getNotifications(): Promise<Notification[]> {
    const response = await api.get<Notification[]>(API_ROUTES.USERS.NOTIFICATIONS)
    return response.data
  },
  ...
  async updateAiContext(data: Partial<AiContext>): Promise<AiContext> {
    const response = await api.put<AiContext>(API_ROUTES.USERS.AI_CONTEXT, data)
    return response.data
  },
