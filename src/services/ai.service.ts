...
export const aiService = {
  // Interações
  async sendMessage(message: string, contexto?: Record<string, any>): Promise<AiInteraction> {
  ...
  async getHistory(params?: { page?: number; limit?: number }): Promise<AiInteraction[]> {
    const response = await api.get<AiInteraction[]>(API_ROUTES.AI.INTERACT, { params })
    return response.data
  },
