...
const CalendarDay: React.FC<{
  ...
  onClick?: () => void;
  disabled?: boolean;
}> = ({ day, isHeader, isToday, hasEvents, isSelected, onClick, disabled }) => {
  return (
    <button
      onClick={onClick}
...
export function CalendarioCard({
  ...
  const [emptyDayDialogOpen, setEmptyDayDialogOpen] = useState(false);
  ...
  const handleAddFromEmptyDay = () => {
    setEmptyDayDialogOpen(false);
    setEditingEvento(null);
    setFormData({
      titulo: "",
      descricao: "",
      duracaoMinutos: 60,
      dataEvento: selectedDate || new Date(),
      hora: "12:00",
    });
    setCreateDialogOpen(true);
  };
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < startDayOfWeek; i++) {
      daysArray.push(
        <div
          key={`empty-start-${i}`}
          className="col-span-1 row-span-1 h-9 w-9"
        />,
      );
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-[24px] border border-border/40 bg-card p-4 transition-all duration-300 hover:border-indigo-400/50 hover:shadow-lg hover:shadow-indigo-500/5">
        <div
        ...
        >
          ...
          {/* Grid do calendário */}
          <div className="grid grid-cols-7 grid-rows-5 gap-1 place-items-center">
            {renderCalendarDays()}
          </div>
        </div>
      </div>
      ...
      {selectedDate && dayEventos.length > 0 && (
          ...
          <ScrollArea className="max-h-64">
            <div className="space-y-2">
              {eventosOrdenados.map((evento) => (
                <div
                  key={evento.id}
                  className="flex items-center justify-between p-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-all group cursor-pointer"
      ...
      {/* Modal de dia vazio */}
      <Dialog open={emptyDayDialogOpen} onOpenChange={setEmptyDayDialogOpen}>
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <div className="flex gap-2 w-full">
              <Button
                variant="outline"
                className="flex-1"
                onClick={() => setEmptyDayDialogOpen(false)}
              >
                Fechar
              </Button>
              <Button className="flex-1" onClick={handleAddFromEmptyDay}>
