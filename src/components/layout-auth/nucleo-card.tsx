...
export interface NucleoData {
  ...
  color?: "health" | "study" | "finance" | "default";
}
...
const colorMap: Record<NonNullable<NucleoData["color"]>, string> = {
...
const progressColorMap: Record<NonNullable<NucleoData["color"]>, string> = {
...
export function NucleoCard({ nucleo, onClick }: NucleoCardProps) {
  const color = nucleo.color ?? "default";
  const badgeClass = colorMap[color];
  const progressClass = progressColorMap[color];

  return (
    <article
      role="button"
      tabIndex={0}
      aria-label={`Abrir núcleo ${nucleo.name}`}
