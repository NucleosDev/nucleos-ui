...
import { Plus, Table2, Loader2, Zap } from "lucide-react";
...
interface TabelaBlocoCardProps {
  bloco: Bloco;
  nucleoId: string;
  onDelete?: () => void;
  onEdit?: () => void;
...
export function TabelaBlocoCard({ bloco }: TabelaBlocoCardProps) {
    ...
    return (
      <>
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl mb-4 bg-teal-500/10 border border-teal-500/20">
            <Table2 className="h-6 w-6 text-teal-500/60" />
          </div>
          <p className="text-sm font-medium text-foreground/70 mb-1">
            Nenhuma tabela ainda
          </p>
          <p className="text-xs text-muted-foreground/60 max-w-xs mb-5">
            Crie uma planilha com campos personalizados para organizar qualquer tipo de dado.
          </p>
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                ...
                "bg-teal-500 text-white hover:opacity-90 transition-opacity",
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-end gap-1.5">
        <Button
          ...
          onClick={() => setQuickTableOpen(true)}
          disabled={isCreating}
          className="h-7 text-xs text-teal-600 hover:bg-teal-500/8 border border-teal-500/25"
        >
          {isCreating ? (
