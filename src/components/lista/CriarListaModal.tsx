...
interface CriarListaModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (nome: string, tipoLista: TipoLista, metadata?: Record<string, any>) => void;
  initialNome?: string;
...
export function CriarListaModal({
  ...
  initialNome = "", initialTipo,
  titulo = "Nova lista", isSubmitting = false,
}: CriarListaModalProps) {
  const nomeInputRef = useRef<HTMLInputElement>(null);
  const [nome,        setNome]        = useState(initialNome);
  const [tipoLista,   setTipoLista]   = useState<TipoFinanceiro | "">(initialTipo ? mapBackendToFrontend(initialTipo) : "");
  ...
  useEffect(() => {
    if (open) {
      ...
      setOrcamento(""); setLocalCompra("");
      setNomeError(null); setTipoError(null);
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let valid = true;
    if (!nome.trim()) { setNomeError("O nome da lista é obrigatório."); valid = false; }
    if (!tipoLista)   { setTipoError("Selecione um tipo de lista.");    valid = false; }
    if (!valid) {
    ...
    if (orcamento) { const v = parseFloat(orcamento); if (!isNaN(v) && v >= 0) metadata.orcamento = v; }
    if (tipoLista === "compras" && localCompra) metadata.localCompra = localCompra;
    onConfirm(nome.trim(), mapFrontendToBackend(tipoLista!) as TipoLista, metadata);
  };
    ...
    <GlassModal open={open} onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground/70">Tipo da lista *</label>
            ...
            {TIPO_OPTS.map((opt) => {
              ...
              return (
                <button
                  key={opt.value}
                  type="button"
                  onClick={() => { setTipoLista(opt.value); if (tipoError) setTipoError(null); }}
                  disabled={isSubmitting}
                  className={cn(
                  ...
                  style={active ? { background: `${ACCENT}15`, border: `1px solid ${ACCENT}40` } : {}}
                >
                  <span className="text-xs font-semibold" style={active ? { color: ACCENT } : {}}>
                    {opt.label}
                  </span>
